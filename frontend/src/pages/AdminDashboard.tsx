...
import {
  ...
  Bell, ChevronDown, CheckCircle, XCircle, Eye
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#F8FAF9] flex w-full font-sans">
      ...
      <aside className="w-[280px] bg-white border-r border-gray-100 flex flex-col h-screen sticky top-0">
        ...
        <nav className="flex-1 px-4 flex flex-col gap-1.5">
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3.5 rounded-xl font-bold text-[14.5px] transition-colors ${activeTab === 'dashboard' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-600 hover:bg-gray-50'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3.5 rounded-xl font-bold text-[14.5px] transition-colors ${activeTab === 'doctors' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-600 hover:bg-gray-50'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3.5 rounded-xl font-bold text-[14.5px] transition-colors ${activeTab === 'patients' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-600 hover:bg-gray-50'}`}
          >
      ...
      <main className="flex-1 overflow-y-auto h-screen flex flex-col">
        {/* Header */}
        <header className="h-[76px] px-8 flex items-center justify-between border-b border-gray-100 bg-white shrink-0">
        ...
        <div className="p-8 lg:p-10 flex-1">
          <div className="max-w-[1200px] mx-auto">
                ...
                {loading ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                      ...
                      <h3 className="text-3xl font-bold text-gray-900">{stats?.totalDoctors || 0}</h3>
                    </div>
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                      <p className="text-[13px] font-medium text-gray-500 mb-1">Pending Verifications</p>
                      <h3 className="text-3xl font-bold text-orange-600">{stats?.pendingDoctors || 0}</h3>
                    </div>
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                      ...
                      <h3 className="text-3xl font-bold text-emerald-600">{stats?.totalTreatments || 0}</h3>
                    </div>
                  </div>
                )}
              </>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <table className="w-full text-left">
                      <thead className="bg-gray-50/50 border-b border-gray-100">
                        <tr>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Doctor</th>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500">License No</th>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Specialization</th>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Status</th>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500 text-center">Docs</th>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500 text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        {doctors.map((doc: any) => (
                          <tr key={doc.id} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-[14px] text-gray-700">{doc.licenseNumber}</td>
                            <td className="px-6 py-4 text-[14px] text-gray-700">{doc.specialization}</td>
                            <td className="px-6 py-4">
            ...
            {activeTab === 'patients' && (
              <>
                <h1 className="text-2xl font-bold text-gray-900 mb-8">Manage Patients</h1>
                {loading ? (
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <table className="w-full text-left">
                      <thead className="bg-gray-50/50 border-b border-gray-100">
                        <tr>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Patient</th>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500">QR ID</th>
                          <th className="px-6 py-4 text-[13px] font-bold text-gray-500">Date of Birth</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        {patients.map((pat: any) => (
                          <tr key={pat.id} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-[14px] text-gray-700">{pat.qrCodeIdentifier}</td>
