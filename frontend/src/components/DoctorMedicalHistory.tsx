...
export default function DoctorMedicalHistory({ onBack, patient }: DoctorMedicalHistoryProps) {
  ...
  const fetchHistory = async () => {
    try {
      ...
      setRecords(Array.isArray(res.data) ? res.data : res.data?.treatments || []);
    } catch (err: any) {
                            ...
                            {record.prescriptions.map((prescription: any) => (
                              <div key={prescription.id}>
                                <p className="text-[15px] font-bold text-gray-900 mb-1">
                                  {prescription.medicineName || 'Unnamed medicine'}
                                </p>
                                <p className="text-[13.5px] text-gray-600">
                                  {[prescription.dosage, prescription.frequency, prescription.duration, prescription.route, prescription.instructions]
                                    .filter(Boolean)
                                    .join(' | ') || 'No prescription details provided'}
                                </p>
                              </div>
                            ))}
                          </div>
